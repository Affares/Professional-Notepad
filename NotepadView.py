...
class Notepad:
    ...
    def __init__(self, root):
        ...
        self.set_toolbar_event_bindings()
        self.change_font()
    ...
    def set_file_sub_menu(self):
        ...
        self.file.add_command(label='Save as', image=self.save_as_icon, compound=tk.LEFT, accelerator='Alt+S',command=self.save_as)
    ...
    def open_file(self,event=None):
        ...
        try:
            self.open_dialog()
            self.msg,self.base=self.notepadContrller.read_file(self.url)
            self.text_editor.delete(1.0,tk.END)
            self.text_editor.insert(1.0,self.msg)
            self.root.title(self.base)
            self.text_editor.edit_modified(False)  #resetting the state of your text. isliye default value False kar rahe h
    ...
    def save_file(self,event=None):
        try:
            ...
            if self.url=="":
                self.save_dialog()
                self.notepadContrller.save_file(content,self.url)
                self.text_changed=False
        #Without else also this code will work.
            else:
                self.notepadContrller.save_file(content,self.url)
                self.text_changed=False
        except Exception:
    ...
    def save_as(self):
        try:
            content=self.text_editor.get(1.0,"end-1c")
            self.save_dialog()
            self.notepadContrller.save_as(content,self.url)
            self.text_changed = False
        except Exception:
    ...
    def set_edit_sub_menu(self):
        ...
        self.edit.add_command(label='Cut', image=self.cut_icon, compound=tk.LEFT, accelerator='Ctrl+X',command=lambda: self.text_editor.event_generate("<<Cut>>"))
    ...
    def find(self):
        ...
        if matches:
            ...
            messagebox.showwarning("Err","No matches found!")
    def replace(self):
        word=self.find_input.get()
    ...
    def set_toolbar_event_bindings(self):
    ...
    def change_font_color(self):
        ...
        #color_var=tk.colorchooser.askcolor(color='red',title='choose color')
        #if None not in color_var:
         #   self.text_editor.config(fg=color_var[1])
        #else:
         #   messagebox.showinfo("No color chosen","You didn't selected any color")
    ...
    def set_color_sub_menu(self):
        ...
        self.color_dict = {'Light Default ': ('#000000', '#ffffff'), 'Light Plus': ('#474747', '#e0e0e0'),'Dark': ('#c4c4c4', '#2d2d2d'), 'Red': ('#2d2d2d', '#ffe8e8'),'Monokai': ('#d3b774', '#474747'), 'Night Blue': ('#ededed', '#6b9dc2')}
            ...
            if self.text_changed:
                ...
                if mbox == True:
                    ...
                    if self.url=="":
                        ...
                        self.notepadContrller.save_file(content,self.url)
            messagebox.showinfo("HAve a good day!","Thank you for using \"My Notepad\" ")
            self.root.destroy()
